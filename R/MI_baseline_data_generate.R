...
MIyear_baseline_data <- function(year, data) {
  ...
  for (i in un_districts_main_year) {
    ...
    ifelse(mainyearminus2$year[1] == as.numeric(myear) | mainyearminus4$year[1] == as.numeric(myear),
           l1 <- MIprecincts_not_found(temp, main_year, statewide_main_year, statewide_main_minus_two,
...
year_baseline_data_mi(2020, mi_data)

MIyear_baseline_data(2020, mi_data)
MIyear_baseline_data(2016, mi_data)



MIprecincts_not_found <- function(temp, main_year, statewide_main_year, statewide_main_minus_two,
                                  statewide_main_minus_four, dis_name,...) {
  df_org <- main_year %>%
...
MIyear_baseline_data(2008, mi_data)
...
## OR (this is also in a separate R script)
## this was MI_baseline_mods file
...
MIyear_baseline_data <- function(year, data) {
  ...
  contested_main_year <- main_year %>%
    dplyr::filter(.data[["contested"]] == "contested")
  con_districts_main_year <- sort(check_districts(contested_main_year))
...
MIyear_baseline_data(2022, mi_data)
